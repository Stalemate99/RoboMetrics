...
export default function RobotUpdateForm(props) {
  const {
    id: idProp,
    robot: robotModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = robotRecord
      ? { ...initialValues, ...robotRecord }
      : initialValues;
    setName(cleanValues.name);
    setWidth(cleanValues.width);
    setLength(cleanValues.length);
    setHeight(cleanValues.height);
    setSensorType(cleanValues.sensorType);
    setImageUrl(cleanValues.imageUrl);
    setErrors({});
  };
  const [robotRecord, setRobotRecord] = React.useState(robotModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Robot, idProp)
        : robotModelProp;
      setRobotRecord(record);
    };
    queryData();
  }, [idProp, robotModelProp]);
  React.useEffect(resetStateValues, [robotRecord]);
  const validations = {
    name: [{ type: "Required" }],
    width: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Robot.copyOf(robotRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RobotUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || robotModelProp)}
